...
export class QuoteService {
    readonly quotes$ = new BehaviorSubject<Quote[]>([]);
    ...
    getQuotes(limit = 10, skip = 0, fields: Array<keyof User> = []): void {
        ...
        this.http.get<QuoteListServerResponse>(url).subscribe((quotes) => this.quotes$.next(quotes.quotes));
    }

    searchQuotes(query: string): void {
        this.http
            .get<QuoteListServerResponse>(`${this.baseUrl}/search?q=${query}`)
            .subscribe((quotes) => this.quotes$.next(quotes.quotes));
    }
